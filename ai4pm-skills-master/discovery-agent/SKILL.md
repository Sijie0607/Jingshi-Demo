...
>
> **输出路径规范**：所有产物统一存放在 `[客户名称]/` 目录下，文件命名遵循
> `[客户名称]-[阶段名称].html` / `.yaml` 格式。
>
> **视觉规范**：参考 `ai4pm-skills/design.md`，默认浅色模式，内容区宽度 85%。
...
> **路径说明**：以下命令需要在 `ai4pm-skills/` 目录下执行。若当前目录不在此处，请先找到 `unified-report-dashboard/scripts/build_dashboard.py` 的绝对路径后再运行。

```bash
# 在 ai4pm-skills/ 目录下执行：
python3 unified-report-dashboard/scripts/build_dashboard.py \
  "[客户名称]/[客户名称]-dashboard.yaml" \
  "[客户名称]/[客户名称]-统一报告仪表盘.html"
```

若脚本路径不存在，提示用户：「请确认当前工作目录为 ai4pm-skills/，或提供 build_dashboard.py 的完整路径。」
