...
def normalise_relationship(rel: dict) -> dict:
    ...
    if cardinality not in valid_cards:
        cardinality = "1:N"
    return {
        "from_id":     safe_str(rel.get("from")),
        "to_id":       safe_str(rel.get("to")),
...
def normalise_transition(tr: dict) -> dict:
    ...
    return {
        "to_state":         safe_str(tr.get("to_state")),
...
def normalise_workstream(ws: dict) -> dict:
    return {
        ...
        "anchor_objects":   ensure_str_list(ws.get("anchor_objects")),
...
# ══════════════════════════════════════════════════════════════════════════════
def generate_agent_prompt(data: dict, entities: list, action_boundaries: list, workstreams: list) -> str:
    lines = []
    ...
    if data.get("relationships"):
        ...
        for rel in data.get("relationships", []):
            if isinstance(rel, dict):
                from_name = entity_lookup.get(rel.get("from"), rel.get("from"))
                to_name = entity_lookup.get(rel.get("to"), rel.get("to"))
            ...
            for state in ws["states"]:
                ...
                if state["transitions"]:
                    ...
                    for tr in state["transitions"]:
                        lines.append(f"      * --[{tr['trigger_action']}] (核实证据: {tr['required_evidence']})--> 到达状态 {tr['to_state']} (性质: {tr['type_label']})")
        if ws["guardrails"]:
...
def main():
    ...
    # Build workstream lookup for dependency labels
    ws_lookup = {ws["id"]: ws["name"] for ws in workstreams}
    for ws in workstreams:
        for dep in ws["dependency_mapping"]:
            dep["workstream_name"] = ws_lookup.get(dep["workstream"], dep["workstream"])

    # Generate Agent Prompt snippet
    agent_prompt = generate_agent_prompt(data, entities, action_boundaries, workstreams)
